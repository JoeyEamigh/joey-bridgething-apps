...
With more than one app in `apps/` your commands must specify which one: `bun run dev claude-usage`.

Screenshot for the store listing:

```sh
bun run shot claude-usage            # grabs what is on the screen
bun run shot claude-usage --replace  # overwrite
```
...
```sh
bun run bump claude-usage patch -m "Fix the wind direction arrow"
git commit -am "claude-usage: fix the wind direction arrow" && git push
```
